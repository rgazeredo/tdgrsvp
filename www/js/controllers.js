var tdgrsvp = angular.module('tdgrsvp.controllers', ['pickadate']);
    ...
    tdgrsvp.controller('EventsCtrl', function ($scope, $rootScope, $http, $location, $ionicModal, UiEvent) {
        ...
        // $ionicModal.fromTemplateUrl('templates/popup-event-total.html', {
        //         scope: $scope
        //     }).then(function(modal) {
        //         $scope.modalEventTotal = modal;
        //     });

        $ionicModal.fromTemplateUrl('templates/popup-total.html', {
                scope: $scope
            }).then(function(modal) {
                $scope.modalTotal = modal;
            });
        ...
        $scope.showTotal = function() {
            $scope.modalTotal.show();
        };

        $scope.hideTotal = function() {
            $scope.modalTotal.hide();
        };
    ...
    tdgrsvp.controller('NamesCtrl', function ($scope, $rootScope, $http, $stateParams, $ionicModal, UiInfo, UiEvent) {
        ...
        $ionicModal.fromTemplateUrl('templates/popup-extra.html', {
                scope: $scope
            }).then(function(modal) {
                $scope.modalExtra = modal;
            });
        ...
        $scope.hideInfoExtra = function() {
            $scope.modalExtra.hide();
        };

        $scope.showExtra = function() {

            var extraPost = {};
                extraPost.date = window.localStorage.dateEvent;

            $http
            .post(UiInfo.baseUrl +'totalextra', extraPost)
            .success(function(response) {
                if(response.status == 'success')
                {
                    $scope.events = $rootScope.events;
                    $scope.nameData = { name: "Extra "+ (response.total + 1), seat: "", control: "", ui_event_id: $scope.event.id };
                    $scope.name = $scope.nameData;
                    $scope.modalExtra.show();
                }
            });
        ...
        $scope.saveName = function(name) {
            ...
            if($scope.nameData.control == "")
            {
                ...
                if($scope.nameData.control < 1000 || $scope.nameData.control > 2000)
                    ...
                    var seatOld = name.seat;
                    var controlOld = name.control;
        ...
        $scope.saveNameExtra = function(name) {

            if($scope.nameData.control == "" || $scope.nameData.name == "" || $scope.nameData.ui_event_id == "0")
            {
                if($scope.nameData.control == "")
                    alert('Informe o número do controle');

                if($scope.nameData.name == "")
                    alert('Informe o nome do convidado');

                if($scope.nameData.ui_event_id == "0")
                ...
                if($scope.nameData.control < 1000 || $scope.nameData.control > 2000)
                    ...
                    name.name = $scope.nameData.name;
                    name.ui_event_id = $scope.nameData.ui_event_id;
                    name.seat = $scope.nameData.seat;
                    name.control = $scope.nameData.control;
                    name.confirmed = 1;
                    ...
                    $http
                        ...
                        .success(function(response) {
                            ...
                            {
                                case 'success':
                                    $scope.names.push(response.ui_event_code_result);
                                    alert('Dados salvos com sucesso!');
                                    $scope.modalExtra.hide();
                                break;
                                case 'error':
