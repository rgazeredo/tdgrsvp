var tdgrsvp = angular.module('tdgrsvp.services', []);
    
    tdgrsvp.factory('UiInfo', function() {
        return {
            baseUrl : 'http://www.tobecorp.com/terradagaroa/mobile/'
        };
    });

    tdgrsvp.factory('UiEvent', function($rootScope, $http, UiInfo) {
        return {
            ...
            findByDate: function(date) {
                ...
                $http
                    .post(UiInfo.baseUrl +'events', datePost)
                    .success(function(response) {
                        if(response.data.length > 0)
                        {   
                            $rootScope.events = response.data; 
                            $rootScope.total = response.total; 
                        } else {
                            $rootScope.events = []; 
                        }
                    });
    ...
    // tdgrsvp.factory('EmployeeService', function($q) {
